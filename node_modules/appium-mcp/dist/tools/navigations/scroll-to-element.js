...
const getValue = (xpath, expression) => {
...
const transformXPath = (xpath) => {
    ...
    throw new Error(`Unsupported XPath expression: ${xpath}. Supported expressions are: @text, @content-desc, contains(@text, ...), contains(@content-desc, ...)`);
};
const transformLocator = (strategy, selector) => {
    if (strategy === 'id')
...
async function performAndroidScroll(driver, args, direction) {
    // Use UiAutomator scroll gestures for Android
    const scrollDirection = direction === 'up' ? 'scrollBackward' : 'scrollForward';
...
async function performiOSScroll(driver, direction) {
...
export default function scrollToElement(server) {
    server.addTool({
                ...
                try {
                    ...
                    switch (platform) {
                        ...
                        case 'iOS':
                            ...
                            break;
                        default:
                            throw new Error('Unsupported driver type. This tool only supports Android and iOS drivers.');
                    }
                    const element = await driver.findElement(args.strategy, args.selector);
                    return {
                        content: [
