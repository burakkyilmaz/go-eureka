...
export default function bootSimulator(server) {
    server.addTool({
        name: 'boot_simulator',
      ...
      This speeds up subsequent session creation by ensuring the simulator is already running.`,
        parameters: z.object({
            udid: z.string().describe(`The UDID of the iOS simulator to boot.
        ...
        execute: async (args, context) => {
            try {
                ...
                if (!simulator) {
                    throw new Error(`Simulator with UDID "${udid}" not found. Please use select_platform and select_device tools to get a valid UDID.`);
                }
                // Check current state
                    ...
                    content: [
                        {
                            type: 'text',
                            text: `${JSON.stringify({
                                instruction: 'You can now use the install_wda tool to install WDA on the simulator.',
...
//# sourceMappingURL=boot-simulator.js.map