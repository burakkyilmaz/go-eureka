...
async function getLatestWDAVersion() {
    try {
        const response = await axios.get('https://api.github.com/repos/appium/WebDriverAgent/releases/latest', {
            headers: {
        ...
        if (error.response) {
            throw new Error(`Failed to fetch WDA version: ${error.response.status} ${error.response.statusText}`);
        }
        throw error;
    }
}
...
export default function setupWDA(server) {
    server.addTool({
        name: 'setup_wda',
      ...
      `,
        parameters: z.object({
            platform: z
                ...
                .optional()
                .default('ios')
                .describe(`The simulator platform to download WDA for.
          Default is "ios".
        ...
        execute: async (args, context) => {
            try {
                ...
                if (process.platform !== 'darwin') {
                    throw new Error('WebDriverAgent setup is only supported on macOS systems');
                }
                ...
                const zipPath = path.join(versionCacheDir, `WebDriverAgentRunner-Build-Sim-${archStr}.zip`);
                ...
                log.info(`Downloading prebuilt WDA v${wdaVersion} for ${platform} simulator...`);
...
//# sourceMappingURL=setup-wda.js.map