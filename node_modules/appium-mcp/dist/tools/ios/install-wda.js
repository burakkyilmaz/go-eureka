...
async function getLatestWDAVersion() {
    ...
    const versions = await Promise.all(entries.map(async (dir) => {
    ...
    if (filteredVersions.length === 0) {
        throw new Error('No WDA versions found in cache. Please run setup_wda first.');
    }
...
async function installAppOnSimulator(appPath, simulatorUdid) {
    try {
        await execAsync(`xcrun simctl install "${simulatorUdid}" "${appPath}"`);
    }
    catch (error) {
        throw new Error(`Failed to install app on simulator ${simulatorUdid}: ${error}`);
    }
}
async function launchAppOnSimulator(bundleId, simulatorUdid) {
    try {
        await execAsync(`xcrun simctl launch "${simulatorUdid}" "${bundleId}"`);
    }
    catch (error) {
        throw new Error(`Failed to launch app on simulator ${simulatorUdid}: ${error}`);
    }
}
async function getAppBundleId(appPath) {
    try {
        const { stdout } = await execAsync(`/usr/libexec/PlistBuddy -c "Print CFBundleIdentifier" "${path.join(appPath, 'Info.plist')}"`);
        return stdout.trim();
    }
    catch (error) {
...
async function isWDAInstalled(simulatorUdid) {
    try {
        const { stdout } = await execAsync(`xcrun simctl listapps "${simulatorUdid}" --json`);
        const data = JSON.parse(stdout);
        // Check if any app has a bundle ID that looks like WDA
        for (const [bundleId, appInfo] of Object.entries(data)) {
            if (bundleId.includes('WebDriverAgentRunner') ||
                appInfo?.CFBundleName?.includes('WebDriverAgent')) {
                return true;
            }
        }
        return false;
...
async function isWDARunning(simulatorUdid) {
    try {
        const { stdout } = await execAsync(`xcrun simctl listapps "${simulatorUdid}" --json`);
        const data = JSON.parse(stdout);
...
export default function installWDA(server) {
    server.addTool({
        name: 'install_wda',
      ...
      `,
        parameters: z.object({
            simulatorUdid: z
                .string()
                .optional()
                .describe('The UDID of the simulator to install WDA on. If not provided, will use the first booted simulator found.'),
            appPath: z
                .string()
                .optional()
                .describe(`The path to the WDA app bundle (.app file) that be generated by setup_wda tool.
        ...
        execute: async (args, context) => {
            try {
                ...
                if (process.platform !== 'darwin') {
                    throw new Error('WDA installation is only supported on macOS systems');
                }
                ...
                try {
                    ...
                    throw new Error(`WDA app not found at ${appPath}. Please run setup_wda first to download and cache WDA, or provide a valid appPath.`);
                }
                // Get booted simulators
                const bootedSimulators = await getBootedSimulators();
                if (bootedSimulators.length === 0) {
                    throw new Error('No booted simulators found. Please boot a simulator first using boot_simulator tool.');
                }
                ...
                if (!bootedSimulators.includes(targetSimulator)) {
                    throw new Error(`Simulator ${targetSimulator} is not booted. Available booted simulators: ${bootedSimulators.join(', ')}`);
                }
                log.info(`Installing WDA from ${appPath} on simulator ${targetSimulator}...`);
...
//# sourceMappingURL=install-wda.js.map