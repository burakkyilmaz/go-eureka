...
export class SentenceTransformersEmbeddings {
    ...
    async initializeTransformers() {
        ...
        try {
            ...
            throw new Error(`Failed to import @xenova/transformers: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
    ...
    async initializeModel() {
        ...
        try {
            ...
            throw new Error(`Failed to initialize sentence-transformers model: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
        ...
        try {
            ...
            for (let i = 0; i < texts.length; i += batchSize) {
                ...
                if (texts.length > batchSize) {
                    log.info(`Processed ${Math.min(i + batchSize, texts.length)}/${texts.length} documents`);
                }
            }
            return embeddings;
        }
            ...
            throw new Error(`Failed to generate document embeddings: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
}
//# sourceMappingURL=sentence-transformers-embeddings.js.map