 ...
 */
function formatMultipleAndroidDevicesResponse(devices) {
 ...
 */
function formatSingleAndroidDeviceResponse(deviceUdid) {
    const platformInfo = `Android platform selected (Found device: ${deviceUdid})`;
    const nextSteps = `Found 1 Android device: ${deviceUdid}\n\n` +
 ...
 */
function formatIOSDeviceTypePrompt() {
    return {
        content: [
            {
                type: 'text',
 ...
 */
async function getIOSDevices(iosDeviceType) {
    const iosManager = IOSManager.getInstance();
    const devices = await iosManager.getDevicesByType(iosDeviceType);
    if (devices.length === 0) {
        const deviceTypeText = iosDeviceType === 'simulator' ? 'simulators' : 'real devices';
        const helpText = iosDeviceType === 'simulator'
 ...
 */
function formatMultipleIOSDevicesResponse(devices, iosDeviceType) {
    const deviceList = devices
        .map((device, index) => `  ${index + 1}. ${device.name} (${device.udid})${device.state ? ` - ${device.state}` : ''}`)
        .join('\n');
    const deviceTypeText = iosDeviceType === 'simulator' ? 'simulators' : 'devices';
    return {
        content: [
            {
                type: 'text',
 ...
 */
function formatSingleIOSDeviceResponse(device, iosDeviceType) {
    const platformInfo = `iOS ${iosDeviceType} selected (Found device: ${device.name} - ${device.udid})`;
    const nextSteps = `Found 1 iOS ${iosDeviceType}: ${device.name} (${device.udid})\n\n` +
 ...
 */
async function handleAndroidPlatformSelection() {
 ...
 */
async function handleIOSPlatformSelection(iosDeviceType) {
    validateMacOSForIOS();
    if (!iosDeviceType) {
...
export default function selectPlatform(server) {
    server.addTool({
      ...
      `,
        parameters: z.object({
            platform: z
                .enum(['ios', 'android'])
                .describe("REQUIRED: The platform chosen by the user - 'android' for Android devices/emulators or 'ios' for iOS devices/simulators. This must be based on the user's explicit choice, NOT a default assumption."),
            iosDeviceType: z
                .enum(['simulator', 'real'])
                .optional()
                .describe("For iOS only: Specify whether to use 'simulator' or 'real' device. REQUIRED when platform is 'ios'."),
        }),
        annotations: {
            readOnlyHint: false,
            openWorldHint: false,
        ...
        execute: async (args, context) => {
            try {
                ...
                log.error(`[select_platform] ${error?.stack || error?.message || String(error)}`);
