...
function transformElementWithLocators(element, locators) {
    ...
    const validLocators = locators.filter((locator) => Array.isArray(locator) &&
        locator.length === 2 &&
 ...
 */
function traverseAndProcessElements(element, sourceXML, isNative, automationName, filters, results) {
    if (!element) {
        return;
    }
    ...
    if (element.children && element.children.length > 0) {
        element.children.forEach(child => traverseAndProcessElements(child, sourceXML, isNative, automationName, filters, results));
    }
}
/**
 ...
 */
export function generateAllElementLocators(sourceXML, isNative = true, automationName, filters = {}) {
...
//# sourceMappingURL=generate-all-locators.js.map