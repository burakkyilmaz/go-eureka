...
export function areAttrAndValueUnique(attrName, attrValue, sourceDoc) {
    ...
    const result = xpathSelect(`//*[@${attrName}="${attrValue.replace(/"/g, '')}"]`, sourceDoc);
...
export function getSuggestedLocators(selectedElement, sourceXML, isNative, automationName) {
    ...
    return sortedLocators.filter((locator) => Array.isArray(locator) &&
        locator.length === 2 &&
...
export function getOptimalXPath(doc, domNode) {
    try {
        ...
        const attrPairsPermutations = attrsForPairs.flatMap((v1, i) => attrsForPairs.slice(i + 1).map(v2 => [v1, v2]));
        const cases = [
        ...
        if (domNode.parentNode) {
            ...
            const childNodes = Array.prototype.slice
                ...
                .filter((childNode) => childNode.nodeType === 1 && childNode.nodeName === domNode.nodeName);
...
export function getOptimalClassChain(doc, domNode) {
    try {
        ...
        for (let attrName of CHECKED_CLASS_CHAIN_ATTRIBUTES) {
            ...
            // If the XPath does not parse, move to the next unique attribute
            try {
                const result = xpathSelect(xpath, doc);
                othersWithAttr = Array.isArray(result)
                    ? result
                    : [];
            }
...
export function getOptimalUiAutomatorSelector(doc, domNode, path) {
    try {
        ...
        for (const [attrName, attrTranslation] of CHECKED_UIAUTOMATOR_ATTRIBUTES) {
            ...
            // If the XPath does not parse, move to the next unique attribute
            try {
                const result = xpathSelect(xpath, newDoc);
                othersWithAttr = Array.isArray(result)
                    ? result
                    : [];
            }
            ...
            else if (!othersWithAttrMinCount ||
...
//# sourceMappingURL=locator-generation.js.map